  ...
  head: () => ({
    meta: [
      { title: "Central de Dashboards — Portal BI Vem" },
      {
        name: "description",
        content:
          "Todos os dashboards do Power BI liberados para o seu perfil, organizados por área.",
      },
      { property: "og:title", content: "Central de Dashboards — Portal BI Vem" },
      {
        property: "og:description",
        content: "Dashboards corporativos publicados no Power BI Service.",
      },
...
function DashboardCenter() {
  ...
  const list = dashboards
    .filter((d) => (activeCategory ? d.category_id === activeCategory.id : true))
    ...
    <div className="mx-auto w-full max-w-[1600px] px-6 py-8 xl:px-10">
      <header>
        <h1 className="font-display text-3xl font-semibold">
          {activeCategory ? activeCategory.name : "Central de dashboards"}
        </h1>
        <p className="mt-1.5 text-sm text-muted-foreground">
          {list.length} dashboard{list.length === 1 ? "" : "s"} disponível
          {list.length === 1 ? "" : "eis"} para o seu perfil.
        </p>
      </header>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <div className="relative w-full max-w-xs">
          ...
          <input
            ...
            className="h-9 w-full rounded-lg border bg-card pl-9 pr-3 text-sm outline-none focus:border-ring"
          />
        </div>
        <div className="flex flex-wrap gap-2">
          <Chip to={{}} label="Todos" active={!cat} />
          {categories.map((c) => (
            <Chip key={c.id} to={{ cat: c.slug }} label={c.name} active={cat === c.slug} />
          ))}
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {list.map((d) => {
          ...
          return (
            <article
              key={d.id}
              className="surface-panel flex flex-col p-5 transition-shadow hover:shadow-lift"
            >
              <div className="flex items-start justify-between">
                ...
                <button
                  onClick={() => toggleFav.mutate({ dashboardId: d.id, isFav })}
              ...
              <h2 className="mt-4 font-display text-base font-semibold">{d.name}</h2>
              <p className="mt-1.5 line-clamp-2 flex-1 text-sm leading-relaxed text-muted-foreground">
                {d.description}
              </p>
              <div className="mt-4 flex items-center gap-2 text-[11px] text-muted-foreground">
                {category && (
                  <span className="rounded-full bg-muted px-2 py-0.5">{category.name}</span>
                )}
                <span>
                  Atualizado{" "}
                  {new Date(d.last_published_at).toLocaleDateString("pt-BR", {
                    day: "2-digit",
                    month: "short",
                  })}
                </span>
      ...
      {!list.length && (
        <p className="mt-16 text-center text-sm text-muted-foreground">
          Nenhum dashboard encontrado para este filtro ou perfil de acesso.
        </p>
      )}
    </div>
  );
